...
var paths = {
	...
	dist: {
		...
		imgDir: config.buildDir + '/image',
		wxssFiles: config.buildDir + '/**/*.wxss'
	},
	tmp: {
		baseDir: 'tmp',
...
function jsRepalce() {
		...
		.pipe(replace('%CLOUD_BUCKET%', config.Bucket))
		.pipe(replace('%CLOUD_REGION%', config.Region))
		.pipe(replace('%API_PATH%', config.rootApi))
		.pipe(replace('%ASSETS_IMG%', config.assetsUrl))
		.pipe(replace('%WEBVIEW_URL%', config.webViewUrl))
...
function sassCompile() {
	...
	return gulp
		...
		.pipe(postcss([lazysprite(lazyspriteConfig),  processors, base64()]))
		.pipe(
			rename({
				extname: '.wxss'
			})
...
gulp.task(
	...
	gulp.series(
		cleanTmp,
		copyBasicFiles,
		gulp.parallel(sassCompile, jsRepalce, imageMin, copyWXML),
		wxmlImgRewrite,
		assetsImgMin,
		assetsServer,
		qcloudCDN,
